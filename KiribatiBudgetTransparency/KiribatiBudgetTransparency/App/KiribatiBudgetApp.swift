...
struct MainTabs: View {
    ...
    var body: some View {
        TabView {
            ...
            MinistriesView()
                .tabItem { Label("Ministries", systemImage: "building.columns") }
            if store.data?.donors2026 != nil {
                DonorsView()
                    .tabItem { Label("Donors", systemImage: "globe.asia.australia") }
            }
            IntegrityView()
...
struct LaunchView: View {
    var body: some View {
        VStack(spacing: 20) {
            BrandMark(size: 72)
            VStack(spacing: 6) {
                Text(Brand.appName).font(.title2.weight(.semibold))
